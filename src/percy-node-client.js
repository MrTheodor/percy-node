 ...
 */
let setup = (() => {
  var _ref = _asyncToGenerator(function* (buildDirs, rootDirs, breakpointsConfig, opt_isDebugEnabled = false) {
    ...
    const resourceManifestDict = gatherBuildResources(percyClient, buildDirs, rootDirs);
    ...
    try {
      ...
      percyBuildPromise = percyClient.createBuild(repo, { resources: resourceManifestArr });
      const buildResponse = yield percyBuildPromise;
      ...
      if (missingResources && missingResources.length > 0) {
        yield uploadMissingResources(percyBuildData.id, missingResources, resourceManifestDict);
      }
    } catch (err) {
 ...
 */
let checkBuildStatus = (() => {
  var _ref2 = _asyncToGenerator(function* (buildId, numRetries, resolve) {
    const response = yield percyClient.getBuild(buildId);
  ...
  return function checkBuildStatus(_x4, _x5, _x6) {
 ...
 */
let finalizeBuild = (() => {
  var _ref3 = _asyncToGenerator(function* (getDiffs = false) {
    logger.log('[percy] Finalizing build...');

    try {
      // These promises need to be processed sequentially, not concurrently.
      const { body: { data: percyBuildData } } = yield percyBuildPromise;
      ...
      yield Promise.all(snapshotResourceUploadPromises);
      // Finalize the build.
      yield percyClient.finalizeBuild(percyBuildData.id);
      ...
      if (getDiffs) {
        yield getBuildPromise(percyBuildData.id);
      }
    } catch (err) {
      handlePercyFailure(err);
    }
 ...
 */
let uploadMissingResources = (() => {
  var _ref4 = _asyncToGenerator(function* (buildId, missingResources, resourceManifestDict) {
    ...
    var promiseGenerator = function () {
      ...
      if (missingResource) {
        ...
        promise.then(response => {
          logger.log(`[percy] Uploaded new build resource: ${resource.resourceUrl}`);
        }, handlePercyFailure);
  ...
  return function uploadMissingResources(_x7, _x8, _x9) {
 ...
 * @const {number}
 */
const MAX_RETRIES_WHEN_PROCESSING = 1000;function snapshot(name, content, opt_breakpoints, opt_enableJs) {
  const enableJs = opt_enableJs || false;
  const defaultBreakpointNames = Object.keys(registeredBreakpoints);
  ...
  percyBuildPromise.then(function (buildResponse) {
    ...
    var snapshotPromise = percyClient.createSnapshot(percyBuildData.id, [htmlResource], {
      name: name,
      widths: widths,
    ...
    snapshotPromise.then(response => {
      ...
      uploadHtml(percyBuildData.id, snapshotId, htmlResource, missingResources, resolveAfterHtmlResourceUploaded);
    }, error => {
      ...
      if (error.statusCode && error.statusCode == 400) {
        console.warn('[percy][WARNING] Bad request error, skipping snapshot: ' + name);
...
function retry(buildId, numRetries, resolve) {
...
}function uploadHtml(buildId, snapshotId, htmlResource, missingResources, callback) {
  if (missingResources.length > 0) {
    ...
    percyClient.uploadResource(buildId, htmlResource.content).then(function () {
      callback();
...
function getWidthsFromBreakpointNames(breakpointNamesList) {
  ...
  for (var i in breakpointNamesList) {
    if (breakpointNamesList.hasOwnProperty(i)) {
      ...
      var breakpointWidth = registeredBreakpoints[breakpointName];
...
function handlePercyFailure(error) {
  isPercyEnabled = false;
  console.error(`[percy][ERROR] API call failed, Percy has been disabled 
